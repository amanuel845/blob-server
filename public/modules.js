...
const jsonBlobUploader = (function() {
  ...
  function uploadFileWorking(file, onProgress, category, extraFields) {
    if (!file) {
      return Promise.reject(new Error('File is required'));
    }
    _smoothedSpeed = 0;
  ...
  function uploadFileA(file, onProgress, category, extraFields) {
  if (!file) {
    return Promise.reject(new Error('File is required'));
  }
  _smoothedSpeed = 0;
  ...
  // ✅ SAFETY NET: Rename the file on the frontend to include the date suffix
  ...
  const formData = new FormData();
  formData.append('image', renamedFile); // Upload the renamed file
  ...
  function uploadFileDirectly(file, onProgress, category, extraFields) {
    ...
    try {
      ...
      if (DEBUG) {
        logger.error('Failed to upload file directly', err);
      }
      throw err;
    }
  }
...
const app = (function() {
  ...
  try {
    ...
    async uploadJsonContent() {
      ...
      try {
        const result = await this.uploader.uploadJson(text, 'blobList.json', null, 'json/others');
        if (DEBUG) {
