var jsonBlobUploader = (function() {
  ...
  // ---- Helper: abortable direct upload ----
  function createDirectUploadRequest(file, presignedUrl, onProgress) {
    ...
    const promise = new Promise((resolve, reject) => {
      ...
      // We'll attach abort to the promise later
      xhr.send(file);
    });

    // Attach abort to the promise itself
    const wrappedPromise = promise;
    wrappedPromise.abort = () => {
      xhr.abort();
      // We'll reject via the promise, but we need a way to signal abort
      // We'll reject in the caller
    };
  ...
  function uploadFileDirectly(file, onProgress, category, extraFields) {
    ...
    return fetch(presignUrl, {
      ...
      .then(({ presignedUrl }) => {
        // Create abortable upload
        const uploadRequest = createDirectUploadRequest(file, presignedUrl, onProgress);
        // Return the promise with abort attached
        const promise = uploadRequest;
        // We need to attach abort to the promise itself
