...
(function() {
  ...
  downloadJsonFile() {
    ...
    try {
      // Optionally validate that it's parseable JSON
      JSON.parse(text);
    } catch (e) {
      // If not valid JSON, still allow download but warn
  ...
  d.codeUpload.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Uploading…';
  
  try {
    const result = await this.uploader.uploadJson(
      text,
      'blobList.json',
    ...
    async uploadFiles(files, onProgress, categories) {
      if (!files || !files.length) return { success: 0, total: 0, failed: [] };
    ...
    populateTable(blobs) {
      ...
      sorted.forEach(blob => {
        ...
        btnDownload.addEventListener('click', () => { blob.downloadUrl; });
  ...
  // Switch to the Code tab (where preview is shown)
  const codeTab = d.tabCode;
  if (codeTab) codeTab.click();
  ...
  if (isImage) {
    // Show image preview
    d.jsonPre.classList.add('hidden');
    d.jsonLoader.classList.add('hidden'); // no loader needed for image
    d.previewModal.classList.remove('hidden');
    d.imagePreview.src = url;
    d.imagePreview.alt = helper.getDisplayName(url);
    
    // Update dropdown selection
    const valueSpan = d.customSelectValue;
    valueSpan.textContent = helper.getDisplayName(url);
    d.fileSelect.dataset.value = url;
    return;
  }
  ...
  d.jsonPre.classList.add('hidden');
  d.jsonLoader.classList.remove('hidden');
  d.previewModal.classList.add('hidden'); // hide image if previously shown
        ...
        try {
          for (const file of files) {
            try {
              ...
              failed.push({ pathname: file.pathname, error: err.message });
            }
          }
          const filtered = this.getFilteredBlobs();
...
if (d.codeUpload) {
d.codeUpload.addEventListener('click', () => this.uploadJsonContent());
}
