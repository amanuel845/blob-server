  (function() {
    ...
    const ui = {
      ...
      // ---- Render dropdown from cached blobs (no fetch) ----
      ...
      // ---- Populate dropdown & table (fetches fresh) ----
      ...
      // ---- Populate table from blobs (with sorting) ----
      populateTable(blobs) {
        const d = this.d;
      ...
      // ---- Delete blob (uses cached list, no extra fetch) ----
      async deleteBlob(pathname, url) {
        ...
        showConfirm(`Delete "${displayName}"?`, async () => {
          ...
          try {
            ...
            this.renderDropdown(this.cachedBlobs);
            this.populateTable(this.cachedBlobs);
            // If the deleted file was selected in the dropdown, clear it
