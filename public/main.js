...
function _findUrlByPathname(pathname) {
  ...
  // ❌ REMOVE THESE LINES (the check is now in uploadFileSmart)
...
const app = (function() {
  ...
  // ✅ Ensure we have the blob list for duplicate checking
  if (!this.cachedBlobs) {
    try {
      this.cachedBlobs = await this.uploader.listBlobs();
    } catch (err) {
      logger.warn('Could not fetch blob list for duplicate check', err);
    }
  }
  
  // ✅ BATCH DUPLICATE CHECK: Ask once for the whole batch
  let overwriteAllDuplicates = true; // Default to true if no duplicates found
  const duplicateFiles = files.filter(file => {
    return window.fileExist(file.lastModified, this.cachedBlobs || []);
  });
  
  if (duplicateFiles.length > 0) {
    const userConfirmed = await new Promise((resolve) => {
      showConfirm(
        `${duplicateFiles.length} file(s) already exist. Do you want to overwrite them all?`,
    ...
    if (!userConfirmed) {
      logger.warn('User chose to skip all duplicate files.');
      ...
      return {
        success: 0,
        total: files.length,
        failed: files.map(file => ({ name: file.name, error: 'Skipped - duplicate' }))
      };
    } else {
      // User confirmed overwrite, set flag to true
  ...
  try {
    for (let i = 0; i < total; i++) {
      ...
      // ✅ SKIP DUPLICATES if user chose to skip them all
      const isDupe = window.fileExist(file.lastModified, this.cachedBlobs || []);
      if (isDupe && !overwriteAllDuplicates) {
        logger.warn(`Skipping duplicate file: ${file.name}`);
        completed++;
        d.currentFileCount.textContent = (i + 1).toString();
        continue;
      }
      ...
      try {
        ...
        completed++;
        // ✅ Add to cached list so subsequent files in the same batch don't trigger a re-check
        if (result && result.pathname) {
          this.cachedBlobs.push(result);
        }
        if (DEBUG) {
          logger.log(`Uploaded ${file.name}:`, result);
        }
      } catch (err) {
        // ✅ REMOVED the USER_CANCELLED_OVERWRITE check here. No more individual prompts!
