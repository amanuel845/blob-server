var jsonBlobUploader = (function() {
  ...
  function uploadSingleFile(formData, category, onProgress) {
...
function uploadFileAdaptive(file, onProgress, category, extraFields, threshold = 4 * 1024 * 1024, chunkSize = 4 * 1024 * 1024) {
  if (!file) return Promise.reject(new Error('File is required'));
...
function uploadFileDirectly(file, onProgress, category, extraFields) {
  ...
  return fetch(presignUrl, {
    ...
    .then(({ presignedUrl }) => {
      // We don't need blobUrl from the server – we'll get it from the PUT response
      return new Promise((resolve, reject) => {
        const xhr = new XMLHttpRequest();
...
function uploadFileSmart(file, onProgress, category, extraFields, threshold = 4 * 1024 * 1024) {
  if (!file) return Promise.reject(new Error('File is required'));
  
  // For small files, use the server‑side upload (limited to ~4.5 MB)
  if (file.size <= threshold) {
    return this.uploadFile(file, onProgress, category, extraFields);
  }
  
  // For larger files, use the direct upload (bypasses server limit)
