...
// ------------------------------------------------------------
// NEW HELPER: Unified Date Suffix Logic
// ------------------------------------------------------------
...
if (req.query.date === 'true') {
  finalFilename = appendDateSuffix(filename, lastModified);
} else {
...
const pathname = `uploads/${category}/${finalFilename}`;
        ...
        const { presignedUrl } = await presignUrl(
          { clientSigningToken, delegationToken },
          {
            ...
            addRandomSuffix: false, // ✅ Unified
            allowOverwrite: true,   // ✅ Unified
          }
        );

        const blobUrl = `https://${BLOB_STORE_ID}.public.blob.vercel-storage.com/${pathname}`;

        // Return the final filename so the frontend knows the exact new name
        return res.status(200).json({ presignedUrl, blobUrl, pathname, filename: finalFilename });
      } catch (error) {
  ...
  if (req.method === 'GET') {
    try {
      ...
      const blobsWithMeta = result.blobs.map(blob => {
        ...
        // 1. If metadata exists (normal and chunked uploads)
...
// Check boolean query to decide if we append the date suffix
if (req.query.date === 'true') {
...
async function handleChunkUpload(fields, files, res) {
  ...
  await put(chunkPath, chunkBuffer, {
    ...
    token: BLOB_READ_WRITE_TOKEN,
    addRandomSuffix: false, // ✅ Unified (Temporary paths are already unique by fileId)
    allowOverwrite: true,   // ✅ Unified
    metadata: {
      fileId,
  ...
  if (uploadedChunks === totalChunks) {
    ...
    // ✅ Unified: Append date suffix to the final filename
    const finalPath = folder + appendDateSuffix(fileName, lastModified);
