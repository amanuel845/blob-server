  ...
  if (req.method === 'POST') {
    ...
    if (req.query.action === 'presign') {
      ...
      try {
        ...
        // ✅ Split the extension and base name
        const lastDotIndex = filename.lastIndexOf('.');
        const baseName = lastDotIndex > -1 ? filename.substring(0, lastDotIndex) : filename;
        const extension = lastDotIndex > -1 ? filename.substring(lastDotIndex) : '';

        // ✅ Create the short date suffix (e.g., 20260907)
        const dateObj = new Date(Number(lastModified));
        const year = dateObj.getUTCFullYear();
        const month = String(dateObj.getUTCMonth() + 1).padStart(2, '0');
        const day = String(dateObj.getUTCDate()).padStart(2, '0');
        const dateSuffix = `${year}${month}${day}`;

        // ✅ Append the short date as a suffix, keeping the extension at the end
        // Final result: IMG_8805_20260907.jpeg
        ...
        const { presignedUrl } = await presignUrl(
          { clientSigningToken, delegationToken },
          {
            ...
            addRandomSuffix: false, // ✅ Fully supported
            allowOverwrite: true,   // ✅ Fully supported
          }
        );
  ...
  try {
    ...
    const blobsWithMeta = result.blobs.map(blob => {
      ...
      if (blob.metadata?.lastModified) {
        ...
        // Example: "IMG_8805_20260907.jpeg" -> "20260907"
